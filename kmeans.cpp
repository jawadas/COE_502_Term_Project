...
int main(int argc, char* argv[]) {
    ...
    data.reserve(1000000); // expected around 1,000,000 points
    ...
    for (int run = 0; run < MAX_RUNS; ++run) {
        ...
        // allocate these once per run (not every iteration)
        ...
        for (iter = 0; iter < MAX_ITERS; ++iter) {
           // -------------------- ASSIGNMENT STEP (PARALLEL) --------------------
            ...
            // reset sums and counts instead of reallocating vectors
    ...
    // flattened array: size N * K  (instead of N separate small vectors)
    vector<float> all_distances(N * K);
    ...
    for (int i = 0; i < N; ++i) {
        for (int c = 0; c < K; ++c) {
            ...
            all_distances[i * K + c] = sqrt(sum_sq);
        }
    }
    ...
    cout << "\nWriting best representative points to 'best_cluster_points.csv'...\n";
    {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            float dist = all_distances[i * K + c];
    ...
    cout << "\nWriting top 10 points per cluster to 'top10_per_cluster.csv'...\n";
    {
        ...
        for (int c = 0; c < K; ++c) {
            ...
            for (int i = 0; i < N; ++i) {
                if (best_cluster[i] == c) {
                    cluster_points.push_back({all_distances[i * K + c], i});
                }
            }
    ...
    cout << "\nWriting cluster statistics to 'cluster_statistics.csv'...\n";
    {
        ...
        for (int c = 0; c < K; ++c) {
            ...
            for (int i = 0; i < N; ++i) {
                if (best_cluster[i] == c) {
                    distances_in_cluster.push_back(all_distances[i * K + c]);
                }
            }
